  ...
  componentDidMount() {
    $(document).ready(()=>{
      $("#orderform").on('click', '.btnSelect', function() {
        ...
        fetch('https://ux2website.herokuapp.com/UX2/app/src/src/api/api.php?action=orderquantity', 
...
class payment extends React.Component {
  ...
  handleSubmit(event){
    event.preventDefault();
    const data = new FormData(event.target);
    fetch('https://ux2website.herokuapp.com/UX/src/src/api/api.php?action=checkout', 
    {
        method: 'POST',
        body: data,
        credentials: 'include'
